...
module Full = struct
    type ('result, 'action, 'model, 'state) t 
        ...
        -> ('result, 'action, 'model, 'state) Snapshot.t Incr.t
end
...
type ('result, 'action, 'model, 'state) t =
    | Constant : 'result -> ('result, Nothing.t, _, _) t
    | Dep :  ('result, 'model) Dep.t -> ('result, Nothing.t, 'model, _) t
    | Full : ('result, 'action, 'model, 'state) Full.t -> ('result, 'action, 'model, 'state) t
    | Compose_similar : 
         ('r1, 'a1, 'model, 'state) t 
       * ('r2, 'a2, 'model, 'state) t 
      -> ('r1 * 'r2, ('a1, 'a2) Either.t, 'model, 'state) t 
    | Compose_disparate :  
         ('r1, 'a1, 'm1, 's1) t 
       * ('r2, 'a2, 'm2, 's2) t
      -> ('r1 * 'r2, ('a1, 'a2) Either.t, 'm1 * 'm2, 's1 * 's2) t

(* OH GOD THIS TYPE SIGNATURE *)
type ('result, 'action, 'model, 'state) eval_type 
  =  old_model:'model Incr.t 
  -> model:'model Incr.t 
  -> inject:('action -> Event.t) 
...
module Similar = struct 
    type ('r1, 'r2, 'a1, 'a2, 'm, 's) compose_type 
      =  eval1:('r1, 'a1, 'm, 's) eval_type 
      -> eval2:('r2, 'a2, 'm, 's) eval_type 
      -> old_model:'m Incr.t
      -> model:'m Incr.t
      -> inject:(('a1, 'a2) Either.t -> Event.t)
      -> ('r1, 'a1, 'm, 's) t 
      -> ('r2, 'a2, 'm, 's) t 
      -> ('r1 * 'r2, ('a1, 'a2) Either.t, 'm, 's) Snapshot.t Incr.t
...
module Disparate = struct 
    type ('r1, 'r2, 'a1, 'a2, 'm1, 'm2, 's1, 's2) compose_type 
      =  eval1:('r1, 'a1, 'm1, 's1) eval_type 
      -> eval2:('r2, 'a2, 'm2, 's2) eval_type 
      -> old_model:('m1 * 'm2) Incr.t
      -> model:('m1 * 'm2) Incr.t
      -> inject:(('a1, 'a2) Either.t -> Event.t)
      -> ('r1, 'a1, 'm1, 's1) t 
      -> ('r2, 'a2, 'm2, 's2) t 
      -> ('r1 * 'r2, ('a1, 'a2) Either.t, 'm1 * 'm2, 's1 * 's2) Snapshot.t Incr.t

    let compose: (_, _, _, _, _, _, _, _) compose_type = fun ~eval1 ~eval2 ~old_model ~model ~inject a b ->
      let inject_a e = inject (Either.first e) in
      let inject_b e = inject (Either.second e) in
      ...
      let apply_action action (state_a, state_b) ~schedule_action = 
        match action with
        | Either.First a_action -> 
          let schedule_action a = schedule_action (Either.first a) in
          Snapshot.apply_action a a_action state_a ~schedule_action, model_b
        | Either.Second b_action -> 
          let schedule_action b = schedule_action (Either.second b) in
          model_a, Snapshot.apply_action b b_action state_b ~schedule_action
      in
      let result = Snapshot.result a, Snapshot.result b in 
      Snapshot.create ~result ~apply_action
end
...
let rec eval: type r a m s . (r, a, m, s) eval_type = 
