...
let draw_history ~global_cursor ~inject ~data:_ ~cursor ~x:_ ~y:_ ~children =
  let on_click = Vdom.Attr.on_click (fun () -> inject (Action.Set_cursor cursor)) in
  let button_attrs =
    if Spacetime_tree.Cursor.equal global_cursor cursor
    then [| on_click; Vdom.Attr.class_name "current" |]
    else [| on_click |]
  in
  let my_node = Vdom.Node.button ~attrs:button_attrs [| Vdom.Node.text "●" |] in
  let children =
    Vdom.Node.div ~attrs:[| Vdom.Attr.class_name "ch" |] (List.to_array children)
  in
...
let view cursor history ~inject =
  ...
  let spacetime =
    Spacetime_tree.traverse history ~f:(draw_history ~inject ~global_cursor:cursor)
  in
  let spacetime =
    Vdom.Node.div ~attrs:[| Vdom.Attr.class_name "history_wrapper" |] [| spacetime |]
  in
  fun window ->
    Vdom.Node.div
...
let create (type r a m) (inner_component : (r, a, m) Ue.Component.t)
  ...
  =
  let open Ue.Incr.Let_syntax in
  Ue.Component.Expert.of_full ~f:(fun ~old_model ~(model : m Model.t Incr.t) ~inject ->
