...
let rec append tree cursor to_insert =
  ...
  | [] ->
    ...
    let children = Int.Map.add_exn tree.children ~key:new_cursor_pos ~data:new_leaf in
  ...
  | x :: rest ->
    ...
    let children = Int.Map.update tree.children x ~f:(Fn.const new_child) in
...
let rec length tree =
  1 + (tree.children |> Map.data |> List.map ~f:length |> List.fold ~init:0 ~f:Int.max)
;;
...
let rec height tree =
  ...
  | 0 -> 1
  | _ ->
    tree.children |> Map.data |> List.map ~f:height |> List.fold_left ~init:0 ~f:( + )
;;
