...
module Queue = struct
  ...
  let dequeue (arr : 'a t) = Js.Optdef.get arr##shift (fun () -> failwith "empty queue")
...
module Mount_point = struct
  ...
  let create_and_mount dom_id vdom =
    ...
    let parent = elem##.parentNode |> Js.Opt.to_option |> Option.value_exn in
  ...
  let update t vdom =
    let vdom = Node.Internal.to_internal_node vdom in
    let patch =
      Virtual_dom.Vdom.Node.Patch.create ~previous:!(t.last_vdom) ~current:vdom
    in
