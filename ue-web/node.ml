...
let verify_attributes attributes =
    ...
    |> List.map ~f:Attr.Internal.to_internal_attr


let create_node node_name : node_creator = fun ?key ?attrs children  ->
    let attr_list = match attrs with 
      | None -> []
      | Some attrs -> verify_attributes attrs 
    in
