...
let start (type a m) (initial_model : m)
    (component : (Node.t, a, m) Component.t) =
  ...
  let module User_action_definition = Event.Define (User_action_handler) in
  ...
  let rec apply_all_actions
      (apply_action : schedule_action:(a -> unit) -> a -> m) =
    if Queue.is_empty queue then Incr.stabilize ()
    else
