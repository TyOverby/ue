...
let tree_leaf_component tree = 
    Component.of_functions 
      ~apply_action:Tree_renderer.apply_action 
      ~view:(Tree_renderer.view tree)

let rec tree_component tree = 
    let open Component.Same_model.Let_syntax in 
    let%map a = tree_leaf_component tree 
    and c = tree_component tree in 
    Vdom.Node.div [| a; c|]

let _basic_tree = Branch ("hi", Nil, Nil)
let basic_tree = Branch ("hi", Branch ("there", Nil, Branch("world", Nil, Nil)), Nil)

let initial_model = 
    { Tree_renderer.opened = true }
;;
